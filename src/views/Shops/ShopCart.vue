...
export default {
    ...
    data(){
        return {
            totalCount: 0,      //存放商品总个数
            moneyCount:null,    //存放商品总价格
            selectFoods: [],    //存放选择的商品名
            showCartView: false //蒙版开关
    ...
    computed: {
        ...
        isEmpty(){
            let empty = true;
            // 要置空是防止在下一次点击时数值累加造成数据不准确，原因是count是一直在增加的，而不是从0 开始
            this.totalCount = 0;
            this.totalPrice = 0;
            this.selectFoods = [];
    ...
    methods: {
        ...
        settlement(){
            this.$store.dispatch("SetOrderInfo",{
                shopInfo: this.shopInfo.rst,    //汉堡王商家信息
                selectFoods: this.selectFoods   //选择的商品
            })
...
.bottomNav-carticon {
    ...
    box-shadow: 0,-0.8 0.533333vw rgba(0, 0, 0, 0.1)
}
