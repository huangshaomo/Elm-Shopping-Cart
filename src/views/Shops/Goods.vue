...
export default {
  ...
  data() {
    return {
      shopInfo: null, //获取到商家信息的所有内容
      menuScroll: {}, //左侧滚动对象
      foodScroll: {}, //右侧滚动对象
      scrollY: 0, //右侧菜单当前滚动到的Y值
      listHeight: [], //12个区的列表高度
      selectedFood: null, //点击的食物
      showFood: false     //存放食物的开关，点击的食物有食物时打开
    };
  },
  created() {
    this.getData();
  ...
  methods: {
    ...
    // 初始化滚动
      ...
      this.foodScroll = new BScroll(this.$refs.foodScroll, {
        probeType: 3, //当屏幕滑动或在momentum滚动动画运行过程中实时派发scroll事件
        click: true
      });
      // 右侧食品栏滚动获取滚动的y值（距离）
    ...
    selectMenu(index) {
    // 点击菜单栏，右边自动滚动到对应位置：获取左边菜单栏索引与右边对应食物列表，
      console.log(index);
